...
class ux_tslib_cObj extends tslib_cObj {
	...
	function cImage($file,$conf) {
		//tkcropthumbs uid 2 lines
		$uid = $this->data[uid];
		$info = $this->getImgResource($file,$conf['file.'],$uid);
	 ...
	 */
	function getImgResource($file,$fileArray,$uid=0) {
			...
			switch($file) {
				...
				default:
					...
					if ($theImage) {
						...
						$hash = t3lib_div::shortMD5($theImage.serialize($fileArray).serialize($maskImages).serialize($uid));
							...
							if (is_array($maskArray) && $GLOBALS['TYPO3_CONF_VARS']['GFX']['im']) {
								...
								if (!file_exists($dest)) {		// Generate!
									...
									if ($m_mask && $m_bgImg) {
										...
										$tempFileInfo = $gifCreator->imageMagickConvert($theImage,$temp_ext,$fileArray['width'],$fileArray['height'],$fileArray['params'],$fileArray['frame'],$options,0,$uid);
								...
								$GLOBALS['TSFE']->tmpl->fileCache[$hash]= $gifCreator->imageMagickConvert($theImage,$fileArray['ext'],$fileArray['width'],$fileArray['height'],$fileArray['params'],$fileArray['frame'],$options,0,$uid);
